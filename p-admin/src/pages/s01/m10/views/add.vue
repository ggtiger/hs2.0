  ...
  <view-dialog :title="title" >
    <div slot="body" style="height: calc(100vh - 107px);">
      <div class="rr-flex-row" style="background:#eee;height: 100%;">
        <div class="left-list rr-flex-1" v-if="!REFTPMDATA&&!ID&&!citem.ID">
          <div>
    ...
    <template slot="footer" class="rr-text-left" v-if="REFTPMDATA!=''">
      ...
      <Tooltip
        ...
        ref="checkTip"
      >
        <Button class="ml5" color="primary">核验</Button>
        <div slot="content">
          <div v-padding="10">
      ...
      <Poptip content="确定撤销核验？" v-per="'RS_M10/A11'" v-if="ISSHOWRECHECK" @confirm="reCheck(ID)">
        <Button class="ml5" color="red" icon="h-icon-close">撤销核验</Button>
      </Poptip>
      <Tooltip
        theme="white"
        v-per="'RS_M10/A12'"
...
export default {
  ...
  mixins: [Add01],
  computed: {
    ...mapDateTable('MAIN', ['REFTPMDATA', 'STATE', 'DEPTNAME', 'PTEMPLATEID', 'DEPTID', 'CREATEID', 'CHECKID', 'CHECKER']),
    ...mapDateTable('PTMP', []),
    ...mapDateTable('TPM', []),
    ...mapDateTable('DEPT', []),
    ...mapDateTable('DTSB', []),
    ...
    ISSHOWSAVE(state, getters, rootState, rootGetters) {
      // 主键、状态空、待提交
      return (!this.STATE || this.STATE === 1 || this.STATE === 12) && (this.isModifyBySelf === false || this.CREATEID == '' || (this.isModifyBySelf && this.CREATEID == this.$store.state.user.userInfo.ID));
    },
    ISSHOWDINVALID() {
  ...
  methods: {
    ...
    async onShow() {
      ...
      } else if (this.citem.ID) {
        await this.$callAction({ action: `${this.storeName}/open`, param: { ID: this.citem.ID }, isBusy: false });
        await this.$callAction({ action: `${this.storeName}/copyAdd`, param: {}, isBusy: false });
        this.showByTemplate();
      } else {
        this.query();
    ...
    async empSel1(INPUT, callback) {
      ...
      await this.$callAction({ action: `${Constants.STORE_NAME}/empSel1`,
        param: {
          INPUT,
          FUNCID: '89f8e81d58c742c2a9a40a34ca09d53a',
          DEPTID: this.DEPTID,
        },
        isBusy: false });
