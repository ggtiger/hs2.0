<template>
  <view-dialog :title="title">
    <div slot="body" >
      ...
      <rs-modal ref="mECertPwd" :autoWidth="true">
     ...
     <rs-modal ref="mResetPwd" :autoWidth="true">
        <div style="padding: 20px;">
          <h4 style="margin-bottom: 15px;">重置电子证书查看密码1</h4>
          ...
          <div style="text-align: right; margin-top: 20px;">
            ...
            <Button color="primary" @click="confirmResetPwd">确认1</Button>
          </div>
        </div>
      </rs-modal>
    </div>
    <template slot="footer">
      ...
      <Button
        ...
        v-per="'LI_M02/A55'"
        color="primary"
        v-if="ISSHOWECERTSIGN"
        @click.native="eCertSign(ID)"
      >电子签发</Button>
      <Button
        class="ml5"
        v-per="'LI_M02/A58'"
        v-if="ISSHOWRESETPWD"
        @click.native="resetECertPwd"
      >重置密码</Button>
...
export default {
  ...
  computed: {
    ...
    ISSHOWECERTSIGN() {
      return this.STATE === 10 && this.ECERTSIGN !== 1;
    },
    ISSHOWRESETPWD() {
      return this.ECERTSIGN === 1;
    },
    FILES: {
      get() {
        let dts = [...this.DTSD];
  ...
  methods: {
    ...
    confirmECertSign() {
      ...
      let ID = this.ID;
      this.$callAction({
        action: `${Constants.STORE_NAME}/eCertSign`,
        successText: '电子签发成功',
        isSuccessBack: true,
        param: { ID, ECERTPWD: this.ecertPwd },
        successCall: () => {
          this.onShow();
        },
      });
    ...
    confirmResetPwd() {
      ...
      let ID = this.ID;
      this.$callAction({
        action: `${Constants.STORE_NAME}/updateECertPwd`,
        successText: this.resetPwd ? '密码修改成功' : '密码已清除',
        isSuccessBack: true,
        param: { ID, ECERTPWD: this.resetPwd },
        successCall: () => {
          this.onShow();
        },
      });
