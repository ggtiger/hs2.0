...
export default {
  ...
  methods: {
    async empSel(INPUT, callback) {
      ...
      await this.$callAction({ action: `${Constants.STORE_NAME}/empSel`, param: {
        INPUT,
      }, isBusy: false });
      callback(this.EMP);
    },
    async custSel(INPUT, callback) {
      ...
      await this.$callAction({ action: `${Constants.STORE_NAME}/custSel`, param: {
        INPUT,
      }, isBusy: false });
      callback(this.CUST);
    },
    async ptmpSel(INPUT, callback) {
      ...
      await this.$callAction({ action: `${Constants.STORE_NAME}/ptmpSel`, param: {
        INPUT,
        DEPTID: this.ADEPTID,
      }, isBusy: false });
      callback(this.PTMP);
    ...
    async deptSel(INPUT, callback) {
      ...
      await this.$callAction({ action: `${Constants.STORE_NAME}/deptSel`, param: {
        INPUT,
      }, isBusy: false });
      callback(this.DEPT);
    },
    async onShow() {
      ...
      try {
        if (this.ID) {
          ...
          if (!this.WTCODE) {
            this.WTCODE = await this.$callAction({ action: `${this.storeName}/getBillCode2`, param: { TCODE: 'WT|%Y%m%d|' }, isBusy: false });
          }
        }
      } finally {
        this.loading = false;
